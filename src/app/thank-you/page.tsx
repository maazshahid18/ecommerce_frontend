      ...
      if (orderNumberFromUrl) {
        try {
          ...
          const data: OrderDetails = {
            ...rawData,
            productPrice: parseFloat(rawData.productPrice || '0'), 
          ...
          setError(`Error loading order details: ${e.message}`);
        ...
        <>
          ...
          <div className="bg-gray-50 p-6 rounded-lg mb-6 border border-gray-200 text-left">
            ...
            {orderDetails.productName && (
                <>
                    <p className="mb-2 text-gray-700"><strong>Product:</strong> {orderDetails.productName}</p>
                    {orderDetails.selectedVariantValue && (
                    <p className="mb-2 text-gray-700"><strong>Variant:</strong> {orderDetails.selectedVariantName}: {orderDetails.selectedVariantValue}</p>
                    )}
                    <p className="mb-2 text-gray-700"><strong>Quantity:</strong> {orderDetails.quantity}</p>
                    <p className="mb-2 text-gray-700"><strong>Price per item:</strong> ${orderDetails.productPrice?.toFixed(2)}</p>
                </>
            )}
            <p className="mb-2 text-gray-700"><strong>Subtotal:</strong> ${orderDetails.subtotal.toFixed(2)}</p>
